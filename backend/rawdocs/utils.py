...
def corrections_to_dataset(output_dir="dataset"):
    ...
    for feedback in MetadataFeedback.objects.all():
        ...
        pdf_path = feedback.file_path  # Ajoute ce champ à MetadataFeedback si absent
        ...
        page = doc.load_page(0)  # Première page pour simplifier
        pix = page.get_pixmap()
        ...
        if annotations:  # Ajouter seulement si des annotations existent
            dataset.append({
                ...
                'image': str(pdf_path),  # Sauvegarde chemin, image non sérialisée pour JSON
                'words': texte,
    ...
    with open(dataset_path, 'w') as f:
        json.dump(dataset, f)
    print(f"✅ Dataset créé avec {len(dataset)} exemples à {dataset_path}")
    return dataset    
...
def update_dataset_on_validation(feedback: MetadataFeedback):
    """Appelée à chaque validation → met à jour le dataset"""
    try:
        # Charger le dataset (sécurisé)
        if DATASET_PATH.exists():
            try:
                ...
                print(f"Fichier dataset corrompu → réinitialisation: {e}")
                dataset = []
        else:
            dataset = []
        ...
        # === Cas 2 : Validé sans changement → AI correcte ===
        ...
        if annotations:
            ...
            print(f"Dataset mis à jour : {len(annotations)} champs ajoutés")
