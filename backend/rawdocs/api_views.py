...
@require_http_methods(["GET"])
@login_required
def get_all_document_annotations(request, document_id):
    """Get all annotations for a document (all pages)"""
    try:
        ...
        for ann in annotations:
            annotations_data.append({
                ...
                'start_pos': ann.start_pos,
                'end_pos': ann.end_pos,
                'text': ann.selected_text,  # Use selected_text as text
                'selected_text': ann.selected_text,
                ...
                'color': ann.annotation_type.color if ann.annotation_type else '#cccccc',
                'confidence': ann.confidence_score,  # Use confidence_score
                'reasoning': ann.ai_reasoning or '',  # Use ai_reasoning
                'is_validated': ann.is_validated,
                'mode': ann.mode,
                'created_by': ann.created_by.username if ann.created_by else 'Unknown',
                'start_xpath': ann.start_xpath,
                'end_xpath': ann.end_xpath,
                'page_number': ann.page.page_number if ann.page else None
            })
...
def save_page_json(request, page_number):
    """Save edited JSON for a specific page"""
    try:
        data = json.loads(request.body)
        json_data = data.get('json_data')
        document_id = data.get('document_id')
        
        if not json_data or not document_id:
            return JsonResponse({
                'success': False,
                'error': 'Missing json_data or document_id'
            }, status=400)
        ...
        page = get_object_or_404(DocumentPage, document=document, page_number=page_number)
        
        # Save the JSON to the page
        page.annotations_json = json_data
        page.annotations_summary = json_data.get('summary', '')
        page.save(update_fields=['annotations_json', 'annotations_summary'])
        
        return JsonResponse({
            'success': True,
            'message': 'Page JSON saved successfully'
        })
...
@require_http_methods(["POST"])
@login_required
def save_document_json(request, document_id):
    """Save edited JSON for the entire document"""
    try:
        data = json.loads(request.body)
        json_data = data.get('json_data')
        ...
        # Save the JSON to the document
        document.global_annotations_json = json_data
        document.global_annotations_summary = json_data.get('summary', '')
